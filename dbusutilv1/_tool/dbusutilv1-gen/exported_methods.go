...
func (g *Generator) genExportedMethods(types strv.Strv) {
	...
	result := make(map[string]dbusutilv1.ExportedMethods)
	for _, typ := range types {
		result[typ] = dbusutilv1.ExportedMethods{}
	}
			...
			ast.Inspect(file, func(node ast.Node) bool {
				...
				if funcDecl.Recv != nil && len(funcDecl.Recv.List) == 1 {
					...
					if isTargetFunc(funcDecl, dbusIdent) {
						...
						result[recvTypeName] = append(result[recvTypeName], dbusutilv1.ExportedMethod{
...
func (g *Generator) genGetExportedMethodsFn(result map[string]dbusutilv1.ExportedMethods) {
	...
	for _, typeName := range typeNames {
		...
		sort.Sort(methods)
		g.printf("func (v *%v) GetExportedMethods() dbusutilv1.ExportedMethods {\n", typeName)
		if len(methods) == 0 {
			g.printf("return nil\n")
		} else {
			g.printf("return dbusutilv1.ExportedMethods{\n")
